  ...
  const ActivityRow = ({
    ...
    const activityMapper = (name, action, target) => {
      if (action === "increased_quota") {
        return <div>{`${name} increased ${target}'s quota`}</div>;
      } else if (action === "archived_team") {
        return <div>{`${name} archived the team ${target}`}</div>;
      }
      return <div>{`${name} added new lead to ${target}`}</div>;
    };
    ...
    return (
      <Row width="100%" style={{ gap: "8px" }}>
        ...
        <Box style={{ gap: "4px" }}>
          <Row flex={1}>{activityMapper(name, action, target)}</Row>
          <Row color="#565656" flex={1}>
            {createdAt}
          </Row>
        </Box>
      </Row>
